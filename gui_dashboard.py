...
class ITDashboardGUI:
    def __init__(self, root, network_manager):
        ...
        # Configure styling to match your server.tool.py dark theme
        style = ttk.Style()
    ...
    def add_router(self):
        hostname = simpledialog.askstring("Input", "Router Hostname:", parent=self.root)
        if hostname:
            ip = simpledialog.askstring("Input", "IP Address:", parent=self.root)
            if ip:
                self.manager.add_device(Router(hostname, ip, "OSPF"))
                self.refresh_device_list()

    def add_switch(self):
        hostname = simpledialog.askstring("Input", "Switch Hostname:", parent=self.root)
        if hostname:
            ip = simpledialog.askstring("Input", "IP Address:", parent=self.root)
            if ip:
                self.manager.add_device(Switch(hostname, ip, 24))
                self.refresh_device_list()
